		...
		if (_enablePhysicsCapsules)
		{
			...
			for (int i = 0; i < _capsules.Count; ++i)
			{
				...
				if (data.IsDataValid && data.IsDataHighConfidence)
				{
					...
					{
						/*
						 * The following two line doesn't work,
						 * I used the code from: 
						 * https://forums.oculusvr.com/t5/Unity-VR-Development/Skeleton-physical-colliders-pose-not-updated-during-hand/m-p/803718/highlight/true#M18366
						 */
						//capsule.CapsuleRigidbody.MovePosition(bone.position);
						//capsule.CapsuleRigidbody.MoveRotation(bone.rotation);

						//revised move operation:
